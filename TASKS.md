...
- [x] 摄像头启动超时（"Timeout starting video source"，根因=多个游戏页面/后台浏览器进程抢占用）：v1=AbortError/占用类错误 1.2s 自动重试一次 + 指引性中文文案；v2=标签页切后台自动释放摄像头、回前台自动重开；启动器（.bat）端口已响应时直接开浏览器短路，不再报 Port already in use
