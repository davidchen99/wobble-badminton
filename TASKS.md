...
- [ ] 连握扣球识别链修复：FistDetector 张开判定放宽（exit 1.6→1.5）、握拳略放宽（enter 1.35→1.4）、连握窗口 350→600ms；rally.smash 取消 0.6s 补刀时限（lastHitter=player 且球仍飞向对面即成扣杀）
- [ ] 引导第三步放宽：1.2 秒内任意两次握拳即通过（教学建概念不考精度），文案明确"握紧→松开→再握紧，连着来两下"
