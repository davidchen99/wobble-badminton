...
- [ ] 预览画中画：触屏默认小窗（约 1/3），点按展开/收起；可拖动到任意角落，位置 localStorage 记忆；桌面端保持点击放大。不做"完全隐藏"（骨架是识别成功的唯一反馈）。
- [ ] 触屏控件：结算/晋级出大按钮弹层（下一关/再战/换赛制，手机可点，电脑同套按钮用鼠标，键盘保留）；左上补暂停图标；触屏设备锁定空手模式、隐藏键盘提示（M/R/C/空格文案）。
- [ ] 竖屏相机取景：aspect<1 时拉远+抬 FOV，保证场地两宽边完整；横屏沿用身后视角（≈电脑体验）。明确不做 90° 侧视旋转：手-人映射要跟着拧、教学需两套，成本大体验怪（用户已接受该论证）。
